...
// Upstream starter must stay thin. Personalized sites may keep intentional
// local shadows of gem-owned paths; allow that when the override ledger exists.
const allowLocalOverrides = exists(".al-folio-overrides.yml");
const starterOnlyPaths = ["_includes", "_layouts", "_sass", "_scripts", "assets/tailwind", "tailwind.config.js", "assets/webfonts"];
for (const forbiddenPath of starterOnlyPaths) {
  if (exists(forbiddenPath) && !allowLocalOverrides) {
